...
for gpo_obj in gpo_list:
    ...
    for setting in gpo_settings_list:
        finding = setting.finding
        if finding:
            finding_str = finding.get_str().replace("\\n","\n")
        else:
            finding_str = ""
...
import os
### Save the data to a CSV file
output_file = "parsed_log_web.csv"
...
os.system(r"csvs-to-sqlite parsed_log_web.csv group3r.db")
os.system("datasette serve group3r.db")
